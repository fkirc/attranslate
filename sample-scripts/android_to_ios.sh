...
ANDROID_TO_ANDROID=( "--srcFile=$ANDROID_EN" "--srcLng=en" "--srcFormat=android-xml" "--targetFormat=android-xml" "--service=google-translate" "--serviceConfig=$SERVICE_ACCOUNT_KEY" "--cacheDir=$CACHE_DIR" )
...
ANDROID_TO_iOS=( "--srcFormat=android-xml" "--targetFormat=ios-strings" "--service=sync-without-translate" "--cacheDir=$CACHE_DIR" )
